...
enum _IMU_MESSAGE_TYPES_ { ACCELERATION = 0, ROTATION_VECTOR = 1 };
...
public:
   ...
   * @exception boost::system::system_error if the serial port read fails.
   */
  void SetCompletedPayloadCallback(
...
private:
   ...
   *
   * @remark This has zero safety and is prone to segment faults. It is on you to ensure that the len matches the
   ...
   * @remark
   */
  inline bool IsStartEncoder(const unsigned char &byte) { return byte == 0xFF; }
   ...
   * @remark
   */
  inline bool ValidateMessage(const unsigned char *messageChecksum, const unsigned char *message,
...
public:
  ...
  /**
   * @brief Overloaded constructor to include hardcoded serialPort implementation.
   *
   * @param [in] serialPort is the hardcoded path to allow mock SerialPort allowing injection of MockSerialPort
   */
  IMUSerialPortReader(std::shared_ptr<SerialPortBase> serialPort, std::string path, unsigned int baudRate);
   ...
   * @return
   *
   * @remark Beware of timing. There will be a delay between when the measurement was produced and real machine time.
   * Use the timestamp in the payloads.
   */
  void InstallVectorCallback(
      std::function<void(std::optional<Raw_RotationVectorWAcc>, std::optional<Raw_Accelerometer>)>);
...
private:
  std::shared_ptr<SerialPortBase> m_serialPort;
